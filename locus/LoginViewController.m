...
- (void)keyboardWasShown:(NSNotification*)notification
{
    ...
    viewFrame.size.height -= (keyboardSize.height+200);
    ...
    NSLog(@"INSIDE KEYBOARD WAS SHOWN");
}
...
- (void)keyboardWillBeHidden:(NSNotification*)notification
{
    ...
    NSLog(@"INSIDE KEYBOARD WAS HIDDEN");
}
...
- (void) globalLogin{
    ...
    @try {
        ...
        if([[_tfEmail text] isEqualToString:@""] || [[_tfPassword text] isEqualToString:@""] ) {
            ...
            NSString *post =[[NSString alloc] initWithFormat:@"user_name=%@&user_password=%@",[_tfEmail text],[_tfPassword text]];
            NSLog(@"PostData: %@",post);
            
            //NSURL *url=[NSURL URLWithString:@"http://localhost/~kediamanav/login/login"];
            NSURL *url=[NSURL URLWithString:@"http://locus-trak.rhcloud.com/login/login"];
