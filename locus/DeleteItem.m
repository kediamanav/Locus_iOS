...
- initWithNames:(NSString *)user_name item:(NSString *)item_name delegate:(id<DeleteItemDelegate>) theDelegate{
    ...
    @autoreleasepool {
        ...
        @try {
            ...
            if ([response statusCode] >=200 && [response statusCode] <300)
            {
                ...
                if(success == 1)
                {
                    NSLog(@"Item successfully deleted from global database");
                    self.success = true;
                }
                else{
                    NSString *error_msg = (NSString *) [jsonData objectForKey:@"error_message"];
                    NSLog(@"Item could not be deleted from global database: %@",error_msg);
                }
        ...
        @catch (NSException * e) {
            NSLog(@"Item could not be deleted. Exception: %@", e);
            [[UIApplication sharedApplication] setNetworkActivityIndicatorVisible:NO];
        }
